...
export interface SvgPortalNode<C extends Component<any> = Component<any>>
  extends PortalNodeBase<C> {
  element: SVGElement;
  elementType: typeof ELEMENT_TYPE_SVG;
}
type AnyPortalNode<C extends Component<any> = Component<any>> =
  | HtmlPortalNode<C>
  | SvgPortalNode<C>;

const validateElementType = (
  domElement: Element,
  ...
  throw new Error(
    `Unrecognized element type "${elementType}" for validateElementType.`,
  );
};
